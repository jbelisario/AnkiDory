...
class GuideMe:
    ...
    def show_hint(self, card: Card) -> None:
        ...
        # Get card content
        question = card.q()  # Use q() instead of question()
        answer = card.a()    # Use a() instead of answer()
        card_content = f"Question: {question}\nAnswer: {answer}"
        
        # Get previous hints from database
        from .db import get_hints
        previous_hints = get_hints(card.id)
        ...
        # Store the hint in database
        from .db import save_hint
